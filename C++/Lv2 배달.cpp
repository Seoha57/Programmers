...
int solution(int N, vector<vector<int> > road, int K) {
    ...
    //index - start, first - destination, second - cost
    ...
    while(!pq.empty()) {
        ...
        //현재 마을과 도로가 연결된 마을 탐색
        for(size_t i = 0; i < map[now].size(); ++i) {
            int next_cost = cost + map[now][i].second;
            int next = map[now][i].first;
            
            //cost가 K보다 크거나 이전 계산값보다 크면 더이상 진행x
            if(next_cost > K || dist[next] < next_cost) continue;
