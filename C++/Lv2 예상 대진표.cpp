...
N명의 참가자는 각각 1부터 N번을 차례대로 배정받습니다. 그리고, 1번↔2번, 3번↔4번, ... , N-1번↔N번의 
참가자끼리 게임을 진행합니다. 각 게임에서 이긴 사람은 다음 라운드에 진출할 수 있습니다. 
...
int solution(int n, int a, int b)
{
    ...
    int battle_b = b;
    //A와 B가 같은 그룹이 될 때 까지
    while(battle_a != battle_b) {
        //그룹 넘버 = (라운드 번호+나머지 / 2) -> (나머지를 더하는 이유 - 1,2번을 같은 그룹으로 만들기 위해)
		battle_a += (battle_a % 2);
        battle_a /= 2;
        battle_b += (battle_b % 2);
        battle_b /= 2;
