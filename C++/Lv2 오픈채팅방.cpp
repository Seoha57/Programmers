...
vector<string> solution(vector<string> record) {
    ...
    vector<vector<string>> pre_answer;
    map<string, string> id_nick;
    
    for(int i = 0; i < record.size(); ++i) {
        vector<string> query;
        vector<string> temp_query;
        ...
        if(query[0] == "Enter") {
            if (id_nick.insert({query[1], query[2]}).second == false)
                id_nick[query[1]] = query[2];
            temp_query.push_back(query[1]);
            temp_query.push_back("님이 들어왔습니다.");
            pre_answer.push_back(temp_query);
        }
        else if(query[0] == "Leave") {
            temp_query.push_back(query[1]);
            temp_query.push_back("님이 나갔습니다.");
            pre_answer.push_back(temp_query);
        }
        else if(query[0] == "Change") {
            if (id_nick.insert({query[1], query[2]}).second == false)
                id_nick[query[1]] = query[2];
        }
    }
    ...
    for(int i = 0; i < pre_answer.size(); ++i) {
        string temp = "";
        temp += (id_nick[pre_answer[i][0]] + pre_answer[i][1]);
